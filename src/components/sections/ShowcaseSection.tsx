    ...
    <section id="showcase" className="section-shell bg-paper text-ink">
      <div className="section-inner">
        <div className="protected-copy mb-16 grid gap-8 md:mb-24 md:grid-cols-[minmax(160px,0.32fr)_1fr]">
          <SectionLabel index="03" label="Showcase" />
          <h2 className="max-w-4xl text-[clamp(2rem,4.8vw,5.2rem)] font-semibold leading-[0.98]">
            Selected moments, paced as a sequence rather than a gallery.
          </h2>
        </div>
        <div className="grid gap-6 md:gap-8">
          {showcases.map((item, index) => (
            <Reveal
              ...
              key={item.title}
            >
              <div className="relative min-h-[260px] overflow-hidden bg-[linear-gradient(135deg,#0b1117,#e9eef0_82%)] md:min-h-full">
                <span className="absolute left-5 top-5 text-xs uppercase text-white/52">{String(index + 1).padStart(2, '0')}</span>
                <div className="absolute inset-x-[12%] top-1/2 h-px bg-white/70 shadow-[0_0_24px_rgba(191,239,255,0.26)]" />
                ...
                <div className="absolute right-[24%] top-[34%] aspect-square w-[20%] rounded-full border border-slate-900/12" />
              </div>
